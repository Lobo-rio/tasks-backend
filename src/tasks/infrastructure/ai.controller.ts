...
export class AiController {
    ...
    async aiSearch(@Query('query') query: string) {
        ...
        try {
            ...
            const tasksContext = JSON.stringify(allTasks.data.map(t => ({ id: t.id, title: t.title, description: t.description })));
            ...
            try {
                ...
                if (parsed.relevantTaskIds && parsed.relevantTaskIds.length > 0) {
                    const filteredTasks = allTasks.data.filter(t => parsed.relevantTaskIds.includes(t.id));
                    return { tasks: filteredTasks, summary: parsed.summary };
                }
                return { tasks: [], summary: parsed.summary || "No relevant tasks found." };
